...
def run(settings):
    ...
    settings.num_workers = 2  # Number of workers for image loading
    ...
    net = segm_models.segm_resnet50(backbone_pretrained=True, topk_pos=settings.segm_topk_pos,
                                    topk_neg=settings.segm_topk_neg, mixer_channels=mixer_channels, device=device)
    ...
    # Optimizer
    optimizer = optim.Adam([{'params': actor.net.segm_predictor.parameters(), 'lr': 1e-3},
                            {'params': actor.net.feat_adjust.parameters(), 'lr': 8e-4}],
                           lr=1e-3)
