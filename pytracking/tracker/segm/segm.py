...
#############################################################################
from pytracking.tracker.segm.FeatureBank import FeatureBank
###########################################################################
...
class Segm(BaseTracker):
    ...
    def track(self, image):
        ...
        if self.rotated_bbox == True:
            new_state = (self.pos[1] - (self.target_sz[1] - 1) / 2, self.pos[0] - (self.target_sz[0] - 1) / 2,
                         self.pos[1] + (self.target_sz[1] - 1) / 2, self.pos[0] - (self.target_sz[0] - 1) / 2,
                         self.pos[1] + (self.target_sz[1] - 1) / 2, self.pos[0] + (self.target_sz[0] - 1) / 2,
                         self.pos[1] - (self.target_sz[1] - 1) / 2, self.pos[0] + (self.target_sz[0] - 1) / 2)
        else:
            new_state = (self.pos[1] - (self.target_sz[1] - 1) / 2, self.pos[0] - (self.target_sz[0] - 1) / 2,
    ...
    def init_segmentation(self, image, bb, init_mask=None):
        ...
        for p in segm_net.feat_adjust.parameters():
            p.requires_grad = False
        ...
        "memory feature by the pseudo mask "
        ##########################################################################################
        f_qury, f_keys, valuespos, valuesneg, f_train0 = segm_net.feat_adjust(test_feat_segm[3], train_feat_segm[3],
                                                                   train_masks[0])
        ####################################################################################
        ...
        if init_mask is None:
            ...
            while iters < 1:
                ...
                segm_pred, _ = segm_net.segm_predictor(f_qury, f_train0, f_keys, valuespos, valuesneg, test_feat_segm,
                                                    train_feat_segm, train_masks, test_dist_map)
                ...
                if self.params.save_mask:
                    ...
                    save_mask(image, bb, None, mask, segm_crop_sz, bb, image.shape[1], image.shape[0],
                              self.params.masks_save_path, self.sequence_name, self.frame_num)
        ...
        "memory feature by the referenced mask"
        ##########################################################################################
        _, _, valuespos, valuesneg, _ = segm_net.feat_adjust(test_feat_segm[3], train_feat_segm[3],
                                                                              mask_gpu)
        self.fb = FeatureBank(self.params.obj_n, self.params.budgets, self.params.device1,
                         update_rate=self.params.update_rate, thres_close=self.params.merge_thr, thres_down=self.params.merge_thrdown)

        self.fb.init_bank(f_keys, valuespos, valuesneg)
        #######################################################################
    ...
    def segment_target(self, image, pos, sz):
        ...
        "feature adjust"
        f_qury, f_keys, valuespos, valuesneg, f_train0 = self.segm_net.feat_adjust(test_feat_segm[3], self.train_feat_segm[3],
                                                                   train_masks[0])
        ################################################################################################################

        # Obtain segmentation prediction
        segm_pred, _ = self.segm_net.segm_predictor(f_qury, f_train0, self.fb.keys, self.fb.valuespos, self.fb.valuesneg, test_feat_segm,
                                            self.train_feat_segm, train_masks, test_dist_map)
        ...
        "momory update"
        ########################################################################################
        mask_pred = F.softmax(segm_pred, dim=1)[:, 0, :, :].unsqueeze(dim=1)
        # mask_pred = (mask_pred > self.params.segm_mask_thr)
        # print(mask_pred.shape)
        mask_pos = mask_pred

        mask_pos = F.interpolate(mask_pos, size=(f_qury.shape[-2], f_qury.shape[-1]))
        mask_neg = 1 - mask_pos

        mask_pos = mask_pos.view(*mask_pos.shape[:2], -1) #B, C, H*W
        mask_neg = mask_neg.view(*mask_neg.shape[:2], -1)
        f_qury = f_qury.view(*f_qury.shape[:2], -1)

        if self.frame_num != 1:
            self.fb.update(f_qury, mask_pos, mask_neg, self.frame_num)
        ##########################################################################################
        ...
        if self.segmentation_task:
            ...
            if self.params.save_mask:
                self.vos_mask_result = save_mask(image, bb, None, mask_real, segm_crop_sz, bb, image.shape[1], image.shape[0],
                          self.params.masks_save_path, self.sequence_name, self.frame_num)
        ...
        if len(cnt_area) > 0 and len(contours) != 0 and np.max(cnt_area) > 50:  # 1000:
            ...
            if not self.params.segm_scale_estimation or pixels_ratio < self.params.segm_pixels_ratio:
                ...
                if self.params.save_mask:
                    self.vos_mask_result = save_mask(image, pred_region, mask, mask_real, segm_crop_sz, bb, image.shape[1],
                                                     image.shape[0],
